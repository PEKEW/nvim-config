...
keymap.set("n", "<leader>wq", "<cmd>x<cr>", { silent = true, desc = "quit current window" })

-- quit current window compulsively
...
-- Move speedup
keymap.set("n", "<leader>j", "5j")
keymap.set("n", "<leader>h", "5h")
keymap.set("n", "<leader>k", "5k")
keymap.set("n", "<leader>l", "5l")
...
keymap.set("n", ",,", "printf('`[%s`]', getregtype()[0])", {
  expr = true,
...
keymap.set("n", "<F1>", "<cmd>set spell!<cr>", { desc = "toggle spell" })
...
vim.api.nvim_create_autocmd("FileType", { pattern = "python", 
    command = "nnoremap <F5> :w<cr>:exec '!clear'<cr>:exec '!python3' shellescape(expand('%:p'), 1)<cr>"})
